...
def create_parser():
    parser = argparse.ArgumentParser(prog='pdftocio', description='Import/output the table of contents of a PDF file.')
    parser.add_argument('pdf_path', metavar='doc.pdf', help='path to the input PDF document')
    parser.add_argument('-t', '--toc', metavar='toc', help='path to the table of contents generated by pdftocgen. if this option is not given, the default is stdin, but if no input is piped or redirected to stdin, this program will instead print the existing ToC of the PDF file')
    ...
    parser.add_argument('-p', '--print', action='store_true', help='when flag is set, print the existing ToC in the input PDF file. this flag is usually not necessary, since it is the default behavior when no input is given')
    parser.add_argument('-H', '--human-readable', action='store_true', help='print the toc in a readable format')
    parser.add_argument('-o', '--out', metavar='file.pdf', help='path to the output file. if this flag is not specified, the default is {input}_out.pdf')
...
def main():
    ...
    for o in opts:
        ...
        if o.long == "--human-readable":
            ...
            if opts.toc:
                try:
                    toc_file = open(opts.toc, "r", encoding=get_file_encoding(opts.toc))
                except IOError as e:
